...
function buildLeveTotBadge(): BadgeDef {
  return customBadge(
    ...
    1,
    (sightings) =>
      sightings.some((s) => s.firstSeenTime && timeToMinutes(s.firstSeenTime) < timeToMinutes('07:00'))
        ? 1
        : 0
  )
}
...
function buildOiseauDeNuitBadge(): BadgeDef {
  ...
  return customBadge(
    ...
    1,
    (sightings) => {
      const hasNocturnalSpecies = sightings.some((s) => nocturnalIds.has(s.birdId))
      const hasLateSighting = sightings.some(
        (s) => s.firstSeenTime && timeToMinutes(s.firstSeenTime) > timeToMinutes('22:00')
      )
...
const TINY_POOL = ['Roitelet huppé', 'Roitelet à triple bandeau', 'Troglodyte mignon', 'Mésange bleue', 'Pouillot véloce']
...
function buildCarnetDeTerrainBadge(): BadgeDef {
  return customBadge(
    ...
    (sightings) => {
      const dates = sightings.filter((s) => s.firstSeenDate).map((s) => s.firstSeenDate!)
...
export function buildThematicBadgeDefs(): BadgeDef[] {
  return [
    ...
    checklistBadge('checklist-pieds-dans-leau', 'terrain', "Pieds dans l'eau", '🦆', WETLAND_POOL, 10),
    checklistBadge('checklist-loup-de-mer', 'terrain', 'Loup de mer', '⚓', PELAGIC_POOL, 5),
    checklistBadge('checklist-alpiniste', 'terrain', 'Alpiniste', '🏔️', MOUNTAIN_POOL, 4),
    checklistBadge('checklist-citadin', 'terrain', 'Citadin', '🏙️', URBAN_POOL, 5),
    checklistBadge('checklist-format-pocket', 'physique', 'Format Pocket', '🔍', TINY_POOL, 3),
    checklistBadge('checklist-poids-lourd', 'physique', 'Poids Lourd', '🏋️', HEAVY_POOL, 4),
    checklistBadge('checklist-feu-artifice', 'physique', "Feu d'artifice", '🎆', COLORFUL_POOL, 4),
    checklistBadge('checklist-shiny-hunter', 'physique', 'Shiny Hunter', '✨', SHINY_POOL, 1),
    buildSeigneurDesCieuxBadge(),
    checklistBadge('checklist-maestro', 'comportement', 'Maestro', '🎵', SINGERS_POOL, 5),
    checklistBadge('checklist-grand-migrateur', 'comportement', 'Grand Migrateur', '🧭', PASSAGE_MIGRANT_POOL, 5),
