...
export function BirdCard({ bird, caught, firstSeenDate, onClick }: BirdCardProps) {
  ...
  return (
    ...
    >
      ...
      <span className={`text-[11px] font-medium leading-tight ${style.textColor}`}>
        {bird.frenchName}
      </span>
      {caught && firstSeenDate ? (
        <span className="text-[10px] text-slate-500 dark:text-slate-400">{firstSeenDate}</span>
      ) : (
